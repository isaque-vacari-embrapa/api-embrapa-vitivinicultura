...
from services.producao_service import find_producao

producao_api = Blueprint("producao_api", __name__)


@producao_api.route("/producoes", methods=["GET"])
@jwt_required()
@cache.cached(query_string=True)
def get_producao():
    """
    Retorna a produção em L. de vinhos, sucos e derivados do Rio Grande do Sul/RS.
    Listagem da produção em L. de vinhos, sucos e derivados do Rio Grande do Sul/RS por ano.
    ---
    operationId:
        Busca por ano de produção
    tags:
    ...
    responses:
        200:
            description: Dados da produção em L. de vinhos, sucos e derivados do Rio Grande do Sul/RS
            schema:
                id: ProducaoResponse
                properties:
                    ano:
