...
from services.processamento_service import find_processamento


processamento_api = Blueprint("processamento_api", __name__)


@processamento_api.route("/processamentos", methods=["GET"])
@jwt_required()
@cache.cached(query_string=True)
def get_processamento():
    """
    Retorna a quantidade em Kg de uvas processadas no Rio Grande do Sul/RS.
    Listagem da quantidade em Kg de uvas processadas no Rio Grande do Sul/RS por tipo e ano.
    ---
    operationId:
        Busca por tipo e ano de processamento
    tags:
    ...
    parameters:
        - in: query
          ...
          default: viniferas
          required: true
          enum: [viniferas,americanas-e-hibridas,uvas-de-mesa,sem-classificacao]
        - in: query
          name: ano
          description: Ano
          type: integer
    ...
    responses:
        200:
            description: Quantidade em Kg de uvas processadas no Rio Grande do Sul/RS
            schema:
                id: ProcessamentoResponse
                properties:
                    tipo:
                        type: string
    ...
    return find_processamento(tipo, ano)
